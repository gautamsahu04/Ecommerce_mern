...
const Homepage = () => {
  ...
  const getTotalCount = async () => {
    try {
      const { data } = await axios.get(
        "https://ecommerce-mern-6j9p.onrender.com/api/product/count-product"
      );
      setTotal(data?.total);
    } catch (error) {
      console.log(error);
  ...
  const filterProduct = async () => {
    try {
      const { data } = await axios.post(
        "https://ecommerce-mern-6j9p.onrender.com/api/product/filter-product",
        {
          checked,
          radio,
        }
                  ...
                  <div className="flex-1 flex flex-col justify-between">
                    ...
                    <div className="mt-4 flex flex-col sm:flex-row gap-3">
                      ...
                      <button
                        ...
                        onClick={() => {
                          setCart([...cart, p]);
                          toast.success("Item add to card  successfully");
                          localStorage.setItem("cart",JSON.stringify([...cart , p]))    // to store cart in localstorage to prevent from re-rendering 
                        }}
                      >
                        Add to Cart
                      </button>
